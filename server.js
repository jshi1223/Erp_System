...
function validateSalesDeliveryReceiptInventory(payload = {}) {
...
async function syncDeliverySerialUnits(connection, recordId, serialUnitIds, req) {
  ...
  const productId = Number(record.product_id || 0) || 0;
  const projectId = Number(record.project_id || 0) || null;
  const delivered = salesRecordShouldDeductInventory(record);
  ...
  // 2) Link the selected units (only this DR's product, and only if still in stock
  // or already this DR's), flipping to 'sold' only when the DR is delivered.
  ...
  for (const uid of selected) {
    await queryDbAsync(connection,
      `UPDATE product_units SET sales_record_id = ?, customer_name = ?,
       ...
       WHERE id = ? AND product_id = ? AND (sales_record_id = ? OR status = 'in_stock')`,
      [id, linkCustomer, projectId, newStatus, uid, productId, id]);
  }
}
  ...
  try {
    ...
    const rows = await queryAsync(
      `SELECT p.*,
       ...
       WHERE p.business_entity_id = ? AND p.is_active = TRUE
       GROUP BY p.id
...
app.get('/api/inventory/warehouses', protectAdmin, async (req, res) => {
  try {
    ...
    const rows = await queryAsync(
      `SELECT *
       FROM warehouses
       WHERE business_entity_id = ? AND is_active = TRUE
  ...
  try {
    ...
    const rows = await queryAsync(
      `SELECT u.*, p.sku, p.product_name, p.category, w.warehouse_code, w.warehouse_name,
       ...
       WHERE u.business_entity_id = ?${extra}
