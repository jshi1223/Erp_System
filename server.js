...
function initApp() {
  ...
  db.query(`DROP TABLE IF EXISTS stock_movements`, (err) => {
    if (err) console.error('Stock movements cleanup error:', err);
  });
  db.query(`DROP TABLE IF EXISTS stock`, (err) => {
    if (err) console.error('Stock cleanup error:', err);
  });
  db.query(`DROP TABLE IF EXISTS warehouses`, (err) => {
    if (err) console.error('Warehouses cleanup error:', err);
  });
  db.query(`DROP TABLE IF EXISTS products`, (err) => {
    if (err) console.error('Products cleanup error:', err);
  });
