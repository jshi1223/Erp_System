...
function createPostgresAppPool(options = {}) {
  ...
  const pool = new Pool({
    ...
    // Drop idle clients before the server (hosted Postgres) silently closes them,
    // which otherwise surfaces as ECONNRESET on the next query after a quiet period.
    idleTimeoutMillis: Number(process.env.PGIDLE_TIMEOUT_MS || 30000),
    // Keep the socket alive across NAT/idle timeouts.
    keepAlive: true,
  });

  // The pg Pool emits 'error' when an *idle* client's connection is dropped by the
  // backend (DB restart, network blip, hosted-PG idle reaping). With no listener,
  // Node treats it as an unhandled error and crashes the whole server. Log and
  // continue — the pool will create a fresh client on the next query.
  pool.on('error', (err) => {
    console.error('[postgres pool] idle client error:', err && err.message ? err.message : err);
  });
